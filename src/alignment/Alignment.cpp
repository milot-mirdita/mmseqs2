...
Alignment::Alignment(const std::string &querySeqDB, const std::string &targetSeqDB,
        ...
        maxSeqLen(par.maxSeqLen), compBiasCorrection(par.compBiasCorrection), compBiasCorrectionScale(par.compBiasCorrectionScale), compBiasCorrectionWLocal(par.compBiasCorrectionWLocal), altAlignment(par.altAlignment), alignmentOutputMode(par.alignmentOutputMode),
...
#pragma omp parallel num_threads(threads)
        {
            ...
            Matcher matcher(querySeqType, maxMatcherSeqLen, m, &evaluer, compBiasCorrection, compBiasCorrectionScale, gapOpen, gapExtend, correlationScoreWeight, zdrop, compBiasCorrectionWLocal);
            ...
            if (realign == true) {
                ...
                realigner = &matcher;
                if (realign_m != NULL) {
                    realigner = new Matcher(querySeqType, maxMatcherSeqLen, realign_m, &evaluer, compBiasCorrection, compBiasCorrectionScale, gapOpen, gapExtend, 0.0, zdrop, compBiasCorrectionWLocal);
                }
            }
