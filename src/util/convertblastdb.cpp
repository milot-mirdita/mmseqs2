...
static void pushNumberedVols(const std::string &base, std::vector<std::string> &out) {
    ...
    if (dp != nullptr) {
        ...
        while ((de = readdir(dp)) != nullptr) {
            ...
            if (std::strcmp(name + (len - 4), ".psq") != 0) {
                continue;
            }
    ...
    if (!numbered.empty()) {
        ...
        for (size_t i = 0; i < numbered.size(); ++i) {
            ...
            if (FileUtil::fileExists((p + ".psq").c_str())) {
                out.push_back(p);
            }
        }
        return;
    ...
    if (hasPlain && FileUtil::fileExists((plainPath + ".psq").c_str())) {
        out.push_back(plainPath);
    }
}

static void findVolumes(const std::string &db, std::vector<std::string> &volumes) {
    if (FileUtil::fileExists((db + ".psq").c_str())) {
        volumes.push_back(db);
        return;
    }

    pushNumberedVols(db, volumes);
    ...
    if (!FileUtil::fileExists(palPath.c_str())) {
        Debug(Debug::ERROR) << "No .psq or .pal found for '" << db << "'\n";
        EXIT(EXIT_FAILURE);
    }

    MemoryMapped pal(palPath, MemoryMapped::WholeFile, MemoryMapped::SequentialScan);
    if (!pal.isValid()) {
        Debug(Debug::ERROR) << "Cannot mmap alias file " << palPath << "\n";
        EXIT(EXIT_FAILURE);
    }
    ...
    if (n >= tlen) {
        for (size_t i = 0; i <= n - tlen; ++i) {
            ...
            for (size_t k = 0; k < tlen; ++k) {
                if (toLower((unsigned char)buf[i + k]) != (unsigned char)needle[k]) {
                    match = false;
                    break;
                }
            }
    ...
    if (pos == (size_t)-1) {
        Debug(Debug::ERROR) << "No DBLIST entry in alias file " << palPath << "\n";
        pal.close();
        EXIT(EXIT_FAILURE);
    }

    std::string baseDir = FileUtil::dirName(palPath);
    ...
    for (const std::string &tok : toks) {
        ...
        if (FileUtil::fileExists((cand + ".psq").c_str())) {
            volumes.push_back(cand);
        } else {
            pushNumberedVols(cand, volumes);
        }
    }

    if (volumes.empty()) {
        Debug(Debug::ERROR) << "No volumes listed in alias " << palPath << "\n";
        EXIT(EXIT_FAILURE);
    }
}
...
static bool readPinOffsets(const unsigned char *pin, size_t pinSize,
                           size_t psqSize, size_t phrSize,
                           std::vector<uint32_t> &hdr, std::vector<uint32_t> &seq) {
    if (pinSize < 12) {
        return false;
    ...
    uint32_t ver = be32(pin + o);
    (void)ver;
    o += 4;

    uint32_t dbt = be32(pin + o);
    o += 4;
    ...
    for (int pad = 0; pad < 8; ++pad) {
        if (tryPinAt(pin, pinSize, tsEnd + (size_t)pad, psqSize, phrSize, hdr, seq)) {
            return true;
        }
    }

    size_t maxProbe = std::min(pinSize, tsEnd + (size_t)4096);
    for (size_t pos = tsEnd; pos + 12 < maxProbe; ++pos) {
        if (tryPinAt(pin, pinSize, pos, psqSize, phrSize, hdr, seq)) {
            return true;
        }
    }
...
static void dumpVolumeToDb(const std::string &base,
                           ...
                           size_t &globalOid,
                           int threadIdx = 0) {
    std::string psqPath = base + ".psq";
    std::string phrPath = base + ".phr";
    std::string pinPath = base + ".pin";

    if (!FileUtil::fileExists(psqPath.c_str()) || !FileUtil::fileExists(phrPath.c_str()) || !FileUtil::fileExists(pinPath.c_str())) {
        Debug(Debug::ERROR) << "Missing .psq/.phr/.pin for '" << base << "'\n";
        EXIT(EXIT_FAILURE);
    }

    MemoryMapped psq(psqPath, MemoryMapped::WholeFile, MemoryMapped::SequentialScan);
    MemoryMapped phr(phrPath, MemoryMapped::WholeFile, MemoryMapped::SequentialScan);
    MemoryMapped pin(pinPath, MemoryMapped::WholeFile, MemoryMapped::SequentialScan);
    if (!psq.isValid() || !phr.isValid() || !pin.isValid()) {
    ...
    if (!readPinOffsets(pin.getData(), pin.mappedSize(), psq.mappedSize(), phr.mappedSize(), hdrOff, seqOff)) {
        Debug(Debug::ERROR) << "Cannot locate offset tables in '" << pinPath << "'\n";
        EXIT(EXIT_FAILURE);
    }

    const unsigned char *psqData = psq.getData();
    const unsigned char *phrData = phr.getData();
    ...
    for (size_t i = 0; i < nseq; ++i) {
        ...
        const unsigned char *blob = (h1 >= h0 && (size_t)h1 <= phr.mappedSize()) ? (phrData + h0) : nullptr;
        size_t blobSize = (h1 >= h0 && (size_t)h1 <= phr.mappedSize()) ? (size_t)(h1 - h0) : 0;
        ...
        std::string seqStr = decodePsqSlice(psqData, s0, s1);
...
int convertblastdb(int argc, const char **argv, const Command &command) {
    ...
    DBWriter seqWriter(par.db2.c_str(), par.db2Index.c_str(), par.threads, par.compressed, Parameters::DBTYPE_AMINO_ACIDS);
    seqWriter.open();
    ...
    findVolumes(par.db1, volumes);

    Debug(Debug::INFO) << "Found " << volumes.size() << " volume(s)\n";
...
// #pragma omp parallel for schedule(static)
    for (size_t vi = 0; vi < volumes.size(); ++vi) {
        ...
        dumpVolumeToDb(volumes[vi], seqWriter, hdrWriter, lookupWriter, mapFp, globalOid, 0);
    }
