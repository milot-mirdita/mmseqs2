...
int expandaln(int argc, const char **argv, const Command& command) {
    ...
    EvalueComputation evaluer(cReader->getAminoAcidDBSize(), &subMat, par.gapOpen, par.gapExtend);
        ...
        for (size_t i = 0; i < resultAbReader->getSize(); ++i) {
            ...
            while (*data != '\0') {
                ...
                for (size_t k = 0; k < resultsBc.size(); ++k) {
                    ...
                    rescoreResultByBacktrace(resultAc, aSeq, cSeq, subMat, compositionBias, par.gapOpen, par.gapExtend);
            ...
            std::sort(resultsAc.begin(), resultsAc.end(), Matcher::compareHits);
